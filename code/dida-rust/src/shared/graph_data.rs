...
impl<G> Eq for ToEdge<G> where G: PartialEq + Eq + PartialOrd + Ord {}
...
impl<G> Eq for FromEdge<G> where G: PartialEq + Eq + PartialOrd + Ord {}
...
impl<G> CommEdge<G>
where
    G: Ord + PartialOrd + PartialOrd + PartialEq + Eq,
{
    pub fn new(c: Community, e: ToEdge<G>) -> CommEdge<G> {
        CommEdge { comm: c, edge: e }
    }
}

fn _example<G: Ord + Copy + Clone>(n: Node<G>) -> FromEdge<G> {
    n.unbind().bind()
}
